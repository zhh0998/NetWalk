        ...
        while len(walk) < self.walk_len:
            ...
            if len(list(g.neighbors(cur))) > 0:
                if rand.random() >= alpha:
                    randomNode=rand.choice(list(g.neighbors(cur)))
                    if(len(list(g.neighbors(cur)))<self.walk_len):
                        walk.append(randomNode)
                    elif (randomNode not in walk):
                        walk.append(randomNode)
                    else:
                        continue
                else:
...
class NetWalk_update:
    ...
    """
    def __init__(self, path, walk_per_node, walk_len, init_percent, snap):
        """
    ...
    def __get_data(self):
        """
        Generate initial edge list for training and list of edge lists in each upcoming snapshot
        :return: initial edge list for training and list of edge lists in each upcoming snapshot
        """

        edges = np.loadtxt(self.data_path, dtype=int, comments='%')                                                     #load edges from edges file of graph
        self.vertices = np.unique(edges)                                                                                #Unique element in edge list as vertices
        init_idx = int(len(edges) * self.init_percent)                                                                  #init_percent % of edges taken initially
        init_edges = edges[:init_idx]
        print("total edges: %d, initial edges: %d; total vertices: %d"
              % (len(edges), len(init_edges), len(self.vertices)))

        #Preparing Snapshots of edges that will come later
        snapshots = []
        current = init_idx
    ...
    def getOnehot(self, walks):
        ...
        if(maxrows<=len(self.vertices)):
            ...
            m=maxrows+1
        n=len(rows)
        coo = coo_matrix((data, (rows, cols)), shape=(m,n))
