...
class Model:
    def __init__(self, activation, dimension, walk_len, nodeNum, gama, lamb,beta, rho, epoch, batch_size, learning_rate, optimizer_type, corrupt_prob):
        ...
        self.data = tf.placeholder(tf.int32, shape=[self.nodeNum, None],name='data')
        ...
        if self.optimizer_type == "adam":
            ...
            self.optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.loss, method="L-BFGS-B",options={'maxiter': 200, 'disp': 0})
        # endregion
    ...
    def clique_embedding_loss(self):
        result = DenoisingAE.autoencoder(self.data, self.corrupt_prob, self.dimension, self.beta, self.rho, self.activation,self.lamb, self.gama)
            ...
            for epoch in tqdm(epochs):
                ...
                if self.optimizer_type == "lbfgs":
                    for i in range(len(batches)):
                        ...
                        self.optimizer.minimize(self.sess, loss_callback=self.print_loss, fetches=[self.loss, self.clique_loss, self.ae_loss, self.kl_loss,self.self_weight_decay_J],feed_dict=feed_dict)
                    ...
                    for i in range(len(batches)):
                        ...
                        cl, ae, kl, weight = self.sess.run([self.clique_loss, self.ae_loss, self.kl_loss, self.self_weight_decay_J],feed_dict=feed_dict)
                        bt_loss += loss_bt
